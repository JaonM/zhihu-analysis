...
import com.ansj.vec.domain.Word;
...
public class Learn {
    ...
    /**
     * By maqiang
     * 增加基于negative sample的训练方式
     */

    private Map<String, Word> wordTable = new HashMap<>();    //负采样表
     ...
     * @throws IOException
     */
    private void trainModel(File file, boolean isNS) throws IOException {
        try (BufferedReader br = new BufferedReader(new InputStreamReader(
            ...
            while ((temp = br.readLine()) != null) {
                ...
                if (!isNS) {
                    ...
                    for (int i = 0; i < strs.length; i++) {
                        Word entry = wordTable.get(strs[i]);
                        if (entry == null) {
                            continue;
                        }
                        // The subsampling randomly discards frequent words while keeping the
                    ...
                    for (int index = 0; index < sentence.size(); index++) {
                        nextRandom = nextRandom * 25214903917L + 11;
                        if (isCbow) {
                            cbowNS(index, sentence, (int) nextRandom % window);
                        } else {
                            skipGramNS(index, sentence, (int) nextRandom % window);
                        }
                    }
                }
     ...
     * @param b
     */
    private void cbowNS(int index, List<Word> sentence, int b) {
        Word word = sentence.get(index);
        ...
        List<Word> w_neg = new ArrayList<>();
        w_neg.add(word);
        initUnigramTable(w_neg, word, count);

        //Negative Sampling
        for (int i = 0; i < w_neg.size(); i++) {
            double f = 0;
            for (c = 0; c < layerSize; c++)
                f += neu1[c] * w_neg.get(i).syn1neg[c];
            if (f <= -MAX_EXP)
                f = 0;
            else if (f >= MAX_EXP)
                f = 1;
                ...
                f = expTable[(int) ((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
            int label;
            if (word.name.equals(w_neg.get(i).name)) {
                label = 0;
            } else {
                label = 1;
            }
            ...
            for (c = 0; c < layerSize; c++) {
                neu1e[c] += g * w_neg.get(i).syn1neg[c];
            }
            // Learn weights hidden -> output
            for (c = 0; c < layerSize; c++) {
                w_neg.get(i).syn1neg[c] += g * neu1[c];
            }
     ...
     */
    @SuppressWarnings("Duplicates")
    private void skipGramNS(int index, List<Word> sentence, int b) {
        int a, c = 0;
        for (a = b; a < window * 2 + 1 - b; a++) {
            if (a == window) {
                continue;
            ...
            Word word = sentence.get(c);
            w_neg.add(sentence.get(index));
            initUnigramTable(w_neg, word, 10);

            //Negative Sampling
            for (int i = 0; i < w_neg.size(); i++) {
                double f = 0;
                for (c = 0; c < layerSize; c++)
                    f += w_neg.get(i).syn0[c] * word.syn1neg[c];
                if (f <= -MAX_EXP)
                    f = 0;
                else if (f >= MAX_EXP)
                    f = 1;
                ...
                // Learn weights hidden -> output
                for (c = 0; c < layerSize; c++) {
                    w_neg.get(i).syn1neg[c] += g * word.syn0[c];
                }
            }
     ...
     */
    @SuppressWarnings("Duplicates")
    private void readVocabNS(File file) throws IOException {
        MapCount<String> mc = new MapCount<>();
        double alphaPow = 3 / 4;
        try (BufferedReader br = new BufferedReader(new InputStreamReader(
                new FileInputStream(file)))) {
        ...
        double sumCount = 0;
        for (Entry<String, Integer> element : mc.get().entrySet()) {
            sumCount += element.getValue();
        }
        sumCount = Math.pow(sumCount, alphaPow);
        for (Entry<String, Integer> element : mc.get().entrySet()) {
            Word word = new Word(layerSize);
            word.name = element.getKey();
            word.freq = element.getValue();
            //频率归一化
            word.len = Math.pow(element.getValue(), alphaPow) / sumCount;
            wordTable.put(element.getKey(), word);
        }
    }
     ...
     * @param count  采样的数量
     */
    private void initUnigramTable(List<Word> neg, Word target, int count) {
        int M = 100000000;
        double m = 1 / M;
        ...
        for (int i = 0; i < count; i++) {
            ...
            double len = 0;
            for (Entry<String, Word> elem : wordTable.entrySet()) {
                len += elem.getValue().len;
                if (len > m * r && !elem.getValue().name.equals(target.name)) {
                    neg.add(elem.getValue());
                    break;
                }
            }
        }
     ...
     * @throws IOException
     */
    public void learFile(File file, boolean isNS) throws IOException {
        readVocabNS(file);
        trainModel(file, isNS);
    }
...
//        learn.saveModel(new File("library/javaVector"));
        ...
        File testFile = new File(Learn.class.getClassLoader().getResource("test.txt").getFile());
